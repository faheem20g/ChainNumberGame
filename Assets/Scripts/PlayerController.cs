...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Effects")]
    public float bounceScaleAmount = 1.3f;
    ...
    [SerializeField] private ParticleSystem negativeEffect;
    void Start()
    {
    ...
    void GrowSnake(int amount)
    {
        ...
        int targetBodyCount = Mathf.Min(currentScore - 1, maxBodyParts);
    ...
    void TriggerBounceEffect()
    {
        ...
        _bounceCoroutine = StartCoroutine(BounceScaleRoutine());
    }

    System.Collections.IEnumerator BounceScaleRoutine()
    {
        if (playerVisualContainer == null) yield break;

        float timer = 0f;
        Vector3 initialScale = Vector3.one;
        Vector3 targetScale = Vector3.one * bounceScaleAmount;

        // Scale Up
        while (timer < bounceDuration / 2f)
        {
            float t = timer / (bounceDuration / 2f);
            playerVisualContainer.localScale = Vector3.Lerp(initialScale, targetScale, t);
            timer += Time.deltaTime;
            yield return null;
        }
        
        playerVisualContainer.localScale = targetScale;
        timer = 0f;
        ...
        {
            float t = timer / (bounceDuration / 2f);
            playerVisualContainer.localScale = Vector3.Lerp(targetScale, initialScale, t);
            timer += Time.deltaTime;
            yield return null;
        }

        playerVisualContainer.localScale = initialScale;
